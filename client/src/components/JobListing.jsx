        ...
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
            ...
            <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
                ...
                {filteredJobs.length > 0 && (
                    <div className='flex items-center justify-center space-x-2 mt-10 gap-1'>
                        <a onClick={() => setCurrentPage(Math.max(1, currentPage-1))} href="#job-list" className='mr-3'>
                            <img src={assets.left_arrow_icon} alt="" />
                        </a>
                        {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_, index) => (
                            <a key={index} href="#job-list">
                                <button onClick={() => setCurrentPage(index+1)} className={`w-10 h-10 flex items-center justify-center border rounded ${currentPage === index+1 ? 'bg-blue-100 text-blue-500 border-blue-400' : 'text-gray-500 border-gray-300'}`}>{index+1}</button>
                            </a>
                        ))}
                        <a onClick={() => setCurrentPage(Math.min(Math.ceil(filteredJobs.length/6), currentPage+1))} href="#job-list" className='ml-1'>
